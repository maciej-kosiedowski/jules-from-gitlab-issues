...
class PRSync:
    def __init__(
        ...
        state_file: str = "data/synced_prs.json",
    ):
            ...
            if not pr.draft and pr.number not in self.synced_prs:
                ...
                for f in files:
                    ...
                    if content_resp.status_code == 200:
                        ...
                        action = (
                            "update"
                            if self.gl_client.file_exists(f.filename)
                            else "create"
                        )
                        actions.append(
                            {
                ...
                if self.gl_client.create_branch(source_branch):
                    if self.gl_client.commit_changes(
                        source_branch, f"Sync from GH PR #{pr.number}", actions
                    ):
                        try:
                            ...
                            self._save_state()
                            logger.info(
                                f"Successfully created GitLab MR for GitHub PR #{pr.number}"
                            )
                        except Exception as e:
                            logger.error(
                                f"Failed to create GitLab MR for PR #{pr.number}: {e}"
                            )
